...
bool populate_keyframe_vector(std::vector<float>& frame){
  	float a;
  	for(int i=0;i<12;i++){
  		if(! (fin>>a) ){
  			return true;
  	...
  	node0->loadFrame(fin,frame);
  	node20->loadFrame(fin,frame);
  	return false;
}
//-----------------------------------------------------------------
void setFrame(std::vector<float>& frame){
		int i = 0;
  	enable_perspective = frame[0];
  	spotlight = frame[1];
  	light = frame[2];
		c_xpos = frame[3];
		c_ypos = frame[4];
		...
		c_zrot = frame[11];
		i+=12;
  	node0->setFrame(i,frame);
  	node20->setFrame(i,frame);
}
//-----------------------------------------------------------------
void interpolate(std::vector<float>& startFrame,std::vector<float>& currFrame,std::vector<float>& endFrame, float ratio){
		currFrame.resize(startFrame.size());
		currFrame[0]=startFrame[0];
		currFrame[1]=startFrame[1];
		if(startFrame[2]==1 || (startFrame[2]==2 && endFrame[2]==0) ) currFrame[2] = 1;
		else if(startFrame[2]==0) currFrame[2] = (startFrame[2] + endFrame[2]) / 2;
		else currFrame[2] = 2;

		for(int i=3 ; i<startFrame.size() ; i++){
			currFrame[i] =  (ratio*endFrame[i] + (1-ratio)*startFrame[i]);	
		}
}
...
void renderGL(void)
{
  if(playback_mode){
  	if(!playback_started){
 			fin.open("keyframes.txt" , std::ios::in);
  		populate_keyframe_vector(startFrame);
  		endKeyFrames = populate_keyframe_vector(endFrame);
  		playback_started=true;

  		playback_begin_time = glfwGetTime();
  	}

		double curr_time =	glfwGetTime();
		double expected_keyframe_number = (curr_time - playback_begin_time)/time_between_keyframes;

		if(expected_keyframe_number - keyframe_number >= 1){
			keyframe_number+=1;
			startFrame=endFrame;
			endFrame.clear();
  		endKeyFrames = populate_keyframe_vector(endFrame);
		...
		double ratio = (expected_keyframe_number - keyframe_number) / time_between_keyframes;
		interpolate(startFrame,currentFrame,endFrame,ratio);
		// if (recording)
  ...
  if(save_keyframe){
  	std::ofstream fout("keyframes.txt" , std::ios::app);

  	fout<<enable_perspective<<" ";
  	fout<<spotlight<<" "<<light<<" ";
		fout<<c_xpos<<" "<<c_ypos<<" "<<c_zpos<<" "<<c_up_x<<" "<<c_up_y<<" "<<c_up_z<<" "<<c_xrot<<" "<<c_yrot<<" "<<c_zrot<<" ";
  	
  	node0->saveFrame(fout);
  	node20->saveFrame(fout);
...
int main(int argc, char** argv)
{
  ...
  window = glfwCreateWindow(1024, 1024, "CS475/CS675 Assignment 4: Animation", NULL, NULL);
  if (!window)
    {
      glfwTerminate();
      return -1;
