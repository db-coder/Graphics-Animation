...
namespace csX75
{
	...
	void HNode::saveFrame(std::ofstream& fout){
		fout<<tx<<" "<<ty<<" "<<tz<<" "<<rx<<" "<<ry<<" "<<rz<<" ";
		for(int i=0;i<children.size();i++){
			children[i]->saveFrame(fout);
		}
	}

	void HNode::loadFrame(std::ifstream& fin,std::vector<float>& frame){
		float a;
		for(int i=0;i<6;i++){
  		fin>>a;
  		frame.push_back(a);
		...
		for(int i=0;i<children.size();i++){
			children[i]->loadFrame(fin,frame);
		}
	}

	void HNode::setFrame(int& frameIndex, std::vector<float>& frame){
		tx = frame[frameIndex];
		ty = frame[frameIndex+1];
		tz = frame[frameIndex+2];
		rx = frame[frameIndex+3];
		ry = frame[frameIndex+4];
		rz = frame[frameIndex+5];
		frameIndex+=6;

		for(int i=0;i<children.size();i++){
			children[i]->setFrame(frameIndex, frame);
		}
		update_matrices();
	}
